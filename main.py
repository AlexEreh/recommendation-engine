...
def main():
    movies_df: pl.DataFrame = (pl.read_csv('dataset/movies_metadata.csv', infer_schema_length=100000)
                               .select(pl.col("title", "overview", "vote_average", "vote_count", "genres")))
    user_ratings_df: pl.DataFrame = (pl.read_csv('dataset/ratings_small.csv')
